...
namespace NetCore.BackendServer.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        await _emailSender.SendEmailAsync(Input.Email, "Xác nhận địa chỉ email của bạn", $@"Vui lòng nhấn vào <a href=""{HtmlEncoder.Default.Encode(callbackUrl)}"">đây</a> để xác nhận địa chỉ email của bạn.");
